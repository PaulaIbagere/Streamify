    ...
    try {
        ...
        const recommendedUsers = await User.find({
            $and: [
               ...
               {$id: {$min: currentUser.friends}}, //exclude current user's friends 
               {isOnboarded: true}
            ]
        })
...
export async function sendFriendRequest(req, res){
    try {
        ...
        if(existingRequest){
            return res.status(404).json({message: "A friend request already existing between you and this user"})
        }
        ...
        console.log("Error in executing the send request controller", error.message);
